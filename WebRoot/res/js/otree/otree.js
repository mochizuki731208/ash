...
OTree = function(){
    ...
    this._icoPath = '/res/js/otree/img/';
    this.root = null;
    this.selectedNode = null;
    this.clickListeners = [];

    //是否级联选中父节点
    this._isAutoCheckParent = true;
    //是否级联选中子节点
    this._isAutoCheckSub = true;
...
OTree.prototype = {
    ...
    initialize: function(info){
        ...
        //是否级联选中父节点
        if (Boolean.isInstance(info['isAutoCheckParent'])) {
            this._isAutoCheckParent = info['isAutoCheckParent'];
        };
        //是否级联选中子节点
        if (Boolean.isInstance(info['isAutoCheckSub'])) {
            this._isAutoCheckSub = info['isAutoCheckSub'];
        };
    ...
    getIsAutoCheckSub: function(){
        return this._isAutoCheckSub;
    },
    getIsAutoCheckParent: function(){
        return this._isAutoCheckParent;
    },
...
OTree.Node.prototype = {
    ...
    getPathCount: function(){
        var i = 0;
        for (var parent = this; parent != null; parent = parent.getParentNode()) {
            i++;
        };
        return i;
    },
    ...
    onCheck: function(){
        if (this.checked !== false) {
            ...
            //级联选中父级节点
            //alert(this.ownerTree.getIsAutoCheckParent());
            if(this.ownerTree.getIsAutoCheckParent()){
    ...
    _subCheck: function(checked){
        ...
        var childNodes = this.getChildNodes();
        //级联选中子节点
        //alert("级联选中子节点"+this.ownerTree.getIsAutoCheckSub());
        if(this.ownerTree.getIsAutoCheckSub()){
    ...
    refurbishIcon: function(){
        ...
        if (childNodes.length > 0) {
            if (this._afterChildrenPaintFlag) {//�Ѿ����ƽڵ�
    ...
    paintLine: function(){
        ...
        for (var i = 1, count = pathNodes.length - 1; i < count; i++) {//i=1: [0]�Ǹ�ڵ�,��ڵ�û��clip
    ...
    paintClipIcoImg: function(){
        ...
        if (this.isRoot()) {
            this['html-element']['clip'].style.display = 'none';//����ʾ��ڵ��clip
        }
    },
