...
function InputSuggest(opt){
	...
	this.opacity = null;//opt.opacity;
	//this.data = opt.data || [];
	//this.aid = opt.aid;
	this.active = null;
	this.visible = false;
	this.init();
	this.refresh(opt.data);
}
InputSuggest.prototype = {
	...
	refresh: function(data) {
		...
		if(this.attr(this.input,'curr_val')!= this.input.value){
			this.container.innerHTML = '';
			for(var i=0,len=this.data.length;i<len;i++){
				var item = this.$C('div');
				this.attr(item, 'class', this.itemCls);
				var info = this.data[i];
				item.innerHTML = //this.data[i]['title'] + " : " + this.data[i]['address'];
					'<span id="title">'
					+ info['title']
					+ '</span>'
					+ '               <span id="address"> 详细地址：<em>'
					+ info['address']
					+ '</em></span>'
					+ '               <span style="display:none;" id="lng">'
					+ info['lng']
					+ '</span>'
			...
			this.attr(this.input,'curr_val',this.input.value);		
		}
	},
	show: function(){
	...
	onMousedown: function(){
		...
		this.on(this.container,'mousedown',function(e){
			$("#sendAddr_panel").html(e.target);//($(this.activeCls));
			_this.input.value = $("#sendAddr_panel #address em").html();//target.innerHTML;
			$("#sendAddr_panel").hide();
			/**
			if(this.aid==0) {
				info['from'] = $("#sendAddr_panel").find("em").html();
				info['from_lng'] = $("#sendAddr_panel").find(".lng").html();
				info['from_lat'] = $("#sendAddr_panel").find(".lat").html();
			} else if(this.aid==1) {
				info['to'] = $("#sendAddr_panel").find("em").html();
				info['to_lng'] = $("#sendAddr_panel").find(".lng").html();
				info['to_lat'] = $("#sendAddr_panel").find(".lat").html();
			} else if(this.aid > 1) {
				for(var i=2; i<this.aid; i++) {
					info['rcvAddr' + i] = $("#sendAddr_panel").find("em").html();
					info['rcvAddr'+i+'_lng'] = $("#sendAddr_panel").find(".lng").html();
					info['rcvAddr'+i+'_lat'] = $("#sendAddr_panel").find(".lat").html();
				}
			}
